...
import { IncomeCategoriesType, IncomeFiltersType } from "../types";
...
const IncomeFiltersModal = ({
  ...
  const filtersFromCache = queryClient.getQueryData<IncomeFiltersType>([
    "incomeFiltersData",
  ]);
  const [filtersData, setFiltersData] = useState<IncomeFiltersType>(
    filtersFromCache || INITIAL_INCOME_FILTERS
  );
  ...
  const handleToggleCategory = (category: string | undefined) => {
    ...
    setFiltersData((prev) => ({
      ...
      toggledCategories: prev.toggledCategories?.includes(
        category as IncomeCategoriesType
      )
        ? prev.toggledCategories?.filter((c) => c !== category)
        : [...(prev.toggledCategories || []), category as IncomeCategoriesType],
    }));
  };
  ...
  const handleSaveFilters = () => {
    ...
    queryClient.setQueryData(["incomeFiltersData"], filtersData);

    handleCloseFiltersModal();
  };

  const handleClearFilters = () => {
    setFiltersData(INITIAL_INCOME_FILTERS);
    queryClient.removeQueries({ queryKey: ["incomeFiltersData"] });
    handleCloseFiltersModal();
  };
...
export default IncomeFiltersModal;
